...
void version(void) {
  ...
  printf("Written by Francisco Ocegueda\n");
}
...
void loadfiles(const char* filename1, const char* filename2, int str_case) {
  ...
  if(str_case == 1){
	  ...
	  int j = 0;
	  for(i = 0; i < count1; i++){
		  while(strings1[i][j] != '\0'){
			  strings1[i][j] = tolower(strings1[i][j]);
			  j++;
		  }
		  j = 0;
	  }
	  ...
	  i = 0;
	  for(i = 0; i < count2; i++){
		  while(strings2[i][j] != '\0'){
			  strings2[i][j] = tolower(strings2[i][j]);
			  j++;
		  }
		  j = 0;
	  }
...
int brief_diff(){
	...
	while( (strcmp(strings1[i], strings2[i]) == 0) && i != count1){
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if(ignorecase){ string_case = 1; }
  loadfiles(files[0], files[1], string_case); 
  if(showbrief && report_identical){
	  if(brief_diff() == 0) { printf("\nFILES ARE DIFFERENT!\n"); }
	  else{ printf("\nFILES ARE THE SAME!\n"); }
	  exit(0);
  }
  if(showbrief){ 
